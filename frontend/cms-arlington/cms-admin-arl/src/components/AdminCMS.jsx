...
const models = {
  ...
  posts: { label: "Posts", fields: ["title","body","category","image_url","featured","status"] },
...
function AdminCMS({ user }) {
  ...
  return <section className="cmsShell">
    <div className="cmsHeader"><div><p className="eyebrow">Content management</p><h2>Community operations</h2></div>{active!=="posts"||user.role==="admin"?<button className="primaryButton" onClick={() => setCreating(!creating)}>+ Add {models[active].label.slice(0,-1)}</button>:null}</div>
    ...
    {creating && <form className="createPanel" onSubmit={create}><h3>Create {models[active].label.slice(0,-1)}</h3><div className="createGrid">{displayFields.filter(f=>!['status','role'].includes(f)).map(field=><label key={field}>{field.replaceAll('_',' ')}{['description','body'].includes(field)?<textarea required value={form[field]||''} onChange={e=>setForm({...form,[field]:e.target.value})}/>:field==='image_url'&&['rewards','posts'].includes(active)?<><input type="url" placeholder="https://example.com/image.jpg" value={String(form.image_url||'').startsWith('data:')?'':form.image_url||''} onChange={e=>setForm({...form,image_url:e.target.value.trim()})}/><span className="uploadButton">{form.image_url?"Image selected":`Or upload ${active==='rewards'?'reward':'post'} image`}<input type="file" accept="image/png,image/jpeg,image/webp" onChange={event=>{const file=event.target.files?.[0];if(!file)return;if(file.size>2*1024*1024){setError('Image must be smaller than 2 MB.');return;}const reader=new FileReader();reader.onload=()=>setForm({...form,image_url:reader.result});reader.readAsDataURL(file);}}/></span></>:<input required={!['website','image_url','company_id','inventory','spotlight_position','featured'].includes(field)} min={field==='inventory'||field.includes('points')?1:undefined} type={field==='starts_at'?'datetime-local':field.includes('id')||field.includes('points')||field==='inventory'||field==='spotlight_position'||field==='featured'?'number':'text'} value={form[field]||''} onChange={e=>setForm({...form,[field]:e.target.value})}/>}</label>)}</div><div className="formActions"><button type="button" className="secondaryButton" onClick={()=>setCreating(false)}>Cancel</button><button className="primaryButton">Save</button></div></form>}
